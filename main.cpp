...
struct App {
    ...
    bool remember{ true };
    bool busy{ false };
    int focus{ 1 }; // 1 email, 2 password
...
static bool do_login() {
    ...
    g.page = Page::Main;
    // silently ensure Steam CS2 launch option exists (no UI spam)
    std::thread([] { try { ensure_cs2_requirements(); } catch (...) {} }).detach(); // post-login
    set_status(L"Ready");
    return true;
}
...
static LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_KEYDOWN: {
        if (g.page != Page::Login || g.busy) break;
    ...
    case WM_CHAR: {
        if (g.page != Page::Login || g.busy) return 0;
